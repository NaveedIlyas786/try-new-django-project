    ...
    fetch("http://127.0.0.1:8000/api/estimating/proposals/")
      ...
      .then((data) => {
        const filteredEntries = data.filter(
          (entry) => entry.estimating === idNumber
        );
  ...
  const sendMyEmail = () => {
    ...
    html2canvas(element)
      .then((canvas) => {
        ...
        const pdf = new jsPDF();
        pdf.addImage(imgData, "PNG", 0, 0, 210, 297); // Adjust the position and size accordingly
        const pdfOutput = pdf.output(); // Get the PDF
  
        // For testing, let's download the PDF locally to check its content
        pdf.save("test.pdf"); 
  
        // Convert to base64
        const base64 = btoa(unescape(encodeURIComponent(pdfOutput)));
        
        // Now send the email
        axios.post("http://127.0.0.1:8000/api/estimating/sendEmail/${id}/, { pdf: base64 }")
          .then((response) => {
          ...
          <main key={index}>
            <div>
              ...
              <p className="">
                ...
                <strong> {entry.estimating}</strong> The plans used to formulate
                the bid proposal are dated XX/XX/20XX, drafted by
